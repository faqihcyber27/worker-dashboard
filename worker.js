    ...
    try {
      ...
      if (url.pathname === "/vms" && request.method === "POST") {
        ...
        await env.DB.prepare(`
          ...
          VALUES (?,?,?,?,?,?,?,?,?,?,?)
        `).bind(
          vm.name, vm.ip, vm.function, vm.cluster, vm.host,
          vm.cpu, vm.memory, vm.disk, vm.os, vm.vlan, vm.environment
        ).run()
      ...
      if (url.pathname.startsWith("/vms/") && request.method === "PUT") {
        ...
        `).bind(
          vm.name, vm.ip, vm.function, vm.cluster, vm.host,
          vm.cpu, vm.memory, vm.disk, vm.os, vm.vlan, vm.environment, id
        ).run()
